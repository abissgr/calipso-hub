...
define([ 'calipso' ], function(Calipso) {
	...
	function checkVATNumber(toCheck, country_code) {
		...
		if (country_code) {
			...
			case "FI":
				...
				break;
			case "FR":
				if (country_code == "FR" || country_code == "GF" || country_code == "PF" || country_code == "TF") {
					valid_country = true;
				}
				break;
			case "GB":
	...
	function BGVATCheckDigit(vatnumber) {
		...
		if (vatnumber.length == 9) {
			...
			var temp = 0;
			for (var i = 0; i < 8; i++)
				temp = temp + Number(vatnumber.charAt(i)) * (i + 1);
			...
			var temp = 0;
			for (var i = 0; i < 8; i++)
				temp = temp + Number(vatnumber.charAt(i)) * (i + 3);
	...
	function CZVATCheckDigit(vatnumber) {
		...
		// Individuals type 2
			...
			// Extract the next digit and multiply by the counter.
			for (var i = 0; i < 7; i++)
				total = total + Number(vatnumber.charAt(i + 1)) * multipliers[i];
	...
	function LTVATCheckDigit(vatnumber) {
		...
		if (vatnumber.length == 9) {
			...
			var total = 0;
			for (var i = 0; i < 8; i++)
				total = total + Number(vatnumber.charAt(i)) * (i + 1);
	...
	Calipso.components.backboneform.VatNumberInput = Calipso.components.backboneform.Text.extend({
		initialize : function(options) {
			...
			options.schema.validators.push(function ensure(value, formValues) {
				console.log("VatNumberInput formValues, country: " + _this.schema.countryFieldName);
				console.log(formValues);
				if (value != null && value != "") {
					var country = formValues[_this.schema.countryFieldName];
					if(country){
						country = country.id;
					}
					var checkVATObject = checkVATNumber(value, country);
					var msg = "";
					if (!checkVATObject.valid_vat){
